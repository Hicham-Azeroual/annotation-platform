...
import { FiHome, FiList, FiFileText, FiClock, FiSettings, FiDatabase, FiUsers, FiBarChart2, FiLogOut } from 'react-icons/fi';
...
const Sidebar = () => {
  ...
  const userNavItems = [
    ...
    { path: '/settings-user', icon: <FiSettings size={20} />, label: 'Settings' },
  ];
  ...
  const adminNavItems = [
    ...
    { path: '/statistics', icon: <FiBarChart2 size={20} />, label: 'Statistics' },
  ...
  return (
    <motion.div
      className="hidden md:flex flex-col w-64 bg-base-100/10 dark:bg-base-100/5 backdrop-blur-xl shadow-[4px_0_15px_-3px_rgba(0,0,0,0.1)] fixed top-0 left-0 h-screen z-20"
      initial={{ x: -100, opacity: 0 }}
      animate={{ x: 0, opacity: 1 }}
      transition={{ duration: 0.5, ease: 'easeOut' }}
    >
      {/* Header with Logo and Name */}
      <div className="p-4 border-b border-base-200/30 flex items-center space-x-3">
        <motion.div
      ...
      <div className="flex flex-col flex-1 justify-between">
        ...
        <div className="p-4 border-t border-base-200/30 space-y-4">
          ...
          >
            ...
            <span className="font-semibold bg-blue-500">Logout</span>
          </motion.button>
        </div>
      </div>
    </motion.div>
