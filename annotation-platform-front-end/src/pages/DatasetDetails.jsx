...
import {
  getDatasetDetails,
  getCoupeTextesByDatasetId,
  unassignAnnotatorFromDataset,
      ...
      <div className="relative z-10">
        ...
        >
          <p className="text-sm text-base-content/80">
            <span className="font-medium text-primary">Text 1:</span>{" "}
            {coupeText.text1}
          </p>
          <p className="text-sm text-base-content/80">
            <span className="font-medium text-secondary">Text 2:</span>{" "}
            {coupeText.text2}
          </p>
            ...
            <div className="mt-3 p-3 rounded-lg bg-primary/10 border border-primary/20">
              <p className="text-sm text-primary font-medium">
                ...
                <span className="text-secondary">
                  {coupeText.annotatedClass}
                </span>
              </p>
              <p className="text-sm text-base-content/70 mt-1">
                Annotated On:{" "}
                {new Date(coupeText.annotationDate).toLocaleString()}
              </p>
            </div>
          ) : (
            <p className="text-sm text-base-content/50 italic mt-3">
              Not yet annotated
            </p>
          )}
        </motion.div>
...
const DatasetDetails = () => {
  ...
  // Fetch coupe textes
  const {
    data: coupeTextesResponse,
    error: coupeTextesError,
    isLoading: isCoupeTextesLoading,
  } = useQuery({
  ...
  const unassignMutation = useMutation({
    mutationFn: ({ datasetId, annotatorId }) =>
      unassignAnnotatorFromDataset(datasetId, annotatorId),
    onSuccess: () => {
      setNotification({
        message: "Annotator unassigned successfully",
        type: "success",
      });
  ...
  const filteredAnnotators =
    coupeTextesResponse?.data?.annotators?.filter(
      (annotator) =>
  ...
  if (coupeTextesError) {
    return (
      <Notification
        message={coupeTextesError.message}
        type="error"
        onClose={() => {}}
      />
    );
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        />
        <motion.div
          className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6"
      ...
      >
        ...
        <div className="relative z-10">
          <motion.h2
            className="text-lg sm:text-xl font-semibold flex items-center text-base-content mb-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.6 }}
          >
            <FiDatabase className="mr-2 text-primary w-5 h-5 sm:w-6 sm:h-6" />
            Dataset Information
          </motion.h2>
          <motion.div
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                <AnimatePresence>
                  {dataset.classes?.map((className, index) => (
                    <motion.span
                      key={`${className}-${index}`}
                      className="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-primary/20 text-primary shadow-md"
      ...
      >
        {/* Search Input */}
        <div className="p-4 sm:p-6 border-b border-base-300/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-primary/5 to-secondary/5">
          <motion.h2
          ...
          >
            <FiType className="mr-2 text-primary w-5 h-5 sm:w-6 sm:h-6" />
            Assigned Annotators
          </motion.h2>
          <motion.div
          ...
          >
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 sm:py-2.5 bg-base-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-sm sm:text-base text-base-content"
              value={searchTerm}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-base-300/10 text-base-content/70 text-xs sm:text-sm">
                ...
                <th className="py-3 sm:py-4 px-4 sm:px-6">Action</th>
              </tr>
            </thead>
            <tbody>
              <AnimatePresence>
        ...
        <motion.p
          className="p-4 sm:p-6 text-sm text-base-content/70 drop-shadow"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 1 }}
        >
