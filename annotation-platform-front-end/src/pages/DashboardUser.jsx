...
const StatsCard = ({ title, value, icon, color }) => {
  ...
  return (
    ...
    >
      <motion.div
        className={`relative bg-base-200/60 dark:bg-base-100/60 backdrop-blur-lg rounded-xl p-4 sm:p-5 md:p-6 shadow-lg border border-base-300 dark:border-base-content/10 hover:border-primary/30 transition-all overflow-hidden group`}
      ...
      >
        ...
        />
        <div
          className={`absolute top-0 right-0 w-16 h-16 bg-${color}-500/10 rounded-bl-full transition-all duration-500 group-hover:w-20 group-hover:h-20`}
        />
...
const LinearProgressBar = ({ progress: rawProgress }) => {
  ...
  const adjustedProgress =
    total > 0 ? (annotated === total - 1 ? 100 : (annotated / total) * 100) : 0;
...
const DashboardUser = () => {
  ...
  const calculateStats = (data) => {
    ...
    summaries.forEach((summary) => {
      const [annotated, totalCoupeTextes] = summary.progress
        .split("/")
        .map(Number);
      const progress =
        totalCoupeTextes > 0
          ? annotated === totalCoupeTextes - 1
            ? 100
            : (annotated / totalCoupeTextes) * 100
          : 0;
  ...
  const filteredTasks = data?.data
    ? data.data
        .filter((summary) => {
          const matchesSearch = summary.datasetName
            .toLowerCase()
        ...
        .sort((a, b) => {
          ...
          const progressA =
            totalA > 0
              ? annotatedA === totalA - 1
                ? 100
                : (annotatedA / totalA) * 100
              : 0;
          const progressB =
            totalB > 0
              ? annotatedB === totalB - 1
                ? 100
                : (annotatedB / totalB) * 100
              : 0;

          if (filters.sortBy === "taskId") {
            return filters.sortOrder === "asc"
              ? a.tasks[0].taskId - b.tasks[0].taskId
              : b.tasks[0].taskId - a.tasks[0].taskId;
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="flex flex-col md:flex-row justify-between items-start md:items-end gap-3 sm:gap-4"
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
      >
        <div>
          ...
          <motion.p
            className="text-sm sm:text-base text-base-content/70 mt-1"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.2 }}
          >
      ...
      >
        <div className="flex flex-col md:flex-row gap-3 sm:gap-4 items-start">
          <motion.div className="relative flex-1 w-full" whileHover={{ y: -2 }}>
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 sm:py-3 bg-base-300 dark:bg-base-200 rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-base-content text-sm sm:text-base"
              value={searchTerm}
          ...
          <motion.button
            ...
            className="flex items-center justify-center px-3 sm:px-4 py-2 sm:py-3 bg-base-300 dark:bg-base-200 hover:bg-base-400 dark:hover:bg-base-300 rounded-lg sm:rounded-xl transition-colors text-base-content text-sm sm:text-base"
        ...
        <AnimatePresence>
          {showFilters && (
            <motion.div
              className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-base-300/10 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8"
            ...
            >
              <div className="flex flex-col">
                ...
                <div className="flex flex-wrap gap-3">
                  {["all", "completed", "inProgress", "unassigned"].map(
                    (status) => (
                      <motion.button
                        key={status}
      ...
      >
        <div className="p-4 sm:p-5 md:p-6 border-b border-base-300/10 flex justify-between items-center bg-gradient-to-r from-primary/5 to-secondary/5">
          <motion.h2
            className="text-lg sm:text-xl font-semibold flex items-center text-base-content drop-shadow-md"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <table className="min-w-full table-fixed">
            <thead>
              <tr className="bg-gradient-to-r from-primary/20 to-secondary/20 text-base-content/90 text-xs sm:text-sm shadow-md">
                <th className="w-[60px] sm:w-[80px] p-3 sm:p-4 text-left font-medium">
                  <FiCheckCircle
                    size={14}
                ...
                <th className="w-[150px] sm:w-[200px] p-3 sm:p-4 text-left font-medium">
                  <FiHardDrive size={14} className="inline mr-2 text-primary" />
                  Dataset Name
                </th>
                <th className="w-[200px] sm:w-[300px] p-3 sm:p-4 text-left font-medium">
                  <FiActivity size={14} className="inline mr-2 text-primary" />
                  Description
                </th>
                <th className="w-[150px] sm:w-[200px] p-3 sm:p-4 text-left font-medium">
                  <FiActivity size={14} className="inline mr-2 text-primary" />
                ...
                <th className="w-[100px] sm:w-[150px] p-3 sm:p-4 text-right font-medium">
                  Actions
                </th>
              </tr>
            </thead>
              ...
              <AnimatePresence>
                {filteredTasks.length === 0 ? (
                  ...
                  filteredTasks.map((task, index) => (
                    ...
                    >
                      <td className="w-[60px] sm:w-[80px] p-3 sm:p-4 font-mono text-primary truncate text-xs sm:text-sm">
                        {task.tasks[0].taskId}
                      </td>
                      <td className="w-[150px] sm:w-[200px] p-3 sm:p-4 font-medium text-base-content truncate text-xs sm:text-sm">
                        {task.datasetName}
                      </td>
                      <td className="w-[200px] sm:w-[300px] p-3 sm:p-4 text-base-content truncate text-xs sm:text-sm">
                        {task.description}
                      </td>
                      <td className="w-[150px] sm:w-[200px] p-3 sm:p-4">
                        <LinearProgressBar progress={task.progress} />
                      </td>
                      <td className="w-[100px] sm:w-[150px] p-3 sm:p-4 text-right">
