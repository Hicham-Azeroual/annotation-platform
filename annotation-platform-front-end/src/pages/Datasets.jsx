...
import {
  FiDatabase, FiEye, FiDownload, FiUserPlus, FiHash,
  FiSearch, FiFilter, FiChevronDown, FiChevronUp,
  FiSliders, FiCheckCircle, FiXCircle, FiActivity,
  FiPlus, FiClock, FiChevronLeft, FiChevronRight,
  FiX, FiUpload,
} from 'react-icons/fi';
...
import { getAllDatasets, getDatasetById, exportDataset, createDataset } from '../api/datasetService';
...
const StatsCard = ({ title, value, icon, color }) => {
  ...
  const gradientColors = [
    '#2563EB', '#10B981', '#9333EA', '#F59E0B', '#6B21A8', '#2563EB',
  ];
...
const Datasets = () => {
    ...
    ? data.data.datasets
        .filter((dataset) => {
          ...
          const matchesStatus =
            ...
            (filters.status === 'notCompleted' && dataset.progress < 100 && dataset.progress > 0) ||
  ...
  const handleDetails = async (id) => {
    try {
      ...
      setNotification({ message: 'Failed to fetch dataset details', type: 'error' });
    }
  };
  ...
  const handleExport = async (id) => {
    try {
      ...
      setNotification({ message: 'Dataset downloaded successfully', type: 'success' });
    } catch (error) {
      setNotification({ message: error.message || 'Failed to download dataset', type: 'error' });
    }
  };
            ...
            >
              <div className="flex flex-col">
                ...
                <div className="flex flex-wrap gap-3">
                  {['all', 'completed', 'notCompleted', 'unassigned'].map((status) => (
                    <motion.button
                      key={status}
            ...
            <thead>
              <tr className="bg-gradient-to-r from-primary/20 to-secondary/20 text-base-content/90 text-sm shadow-md">
                <th className="w-[80px] p-4 text-left font-medium">
                  <FiHash size={16} className="inline mr-2 text-primary" />
                  ID
                </th>
                <th className="w-[200px] p-4 text-left font-medium">
                  <FiDatabase size={16} className="inline mr-2 text-primary" />
                  Name
                </th>
                <th className="w-[150px] p-4 text-left font-medium">
                  <FiActivity size={16} className="inline mr-2 text-primary" />
                  Status
                </th>
                <th className="w-[200px] p-4 text-left font-medium">
                  <FiClock size={16} className="inline mr-2 text-primary" />
                  Progress
                </th>
              ...
              <AnimatePresence>
                {filteredDatasets.length === 0 ? (
                  ...
                  filteredDatasets.map((dataset, index) => (
                    ...
                    >
                      <td className="w-[80px] p-4 font-mono text-primary truncate">{dataset.id}</td>
